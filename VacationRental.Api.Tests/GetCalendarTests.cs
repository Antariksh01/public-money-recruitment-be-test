    ...
    public class GetCalendarTests
    {
        ...
        public async Task GivenCompleteRequest_WhenGetCalendar_ThenAGetReturnsTheCalculatedCalendar()
        {
            ...
            using (var getCalendarResponse = await _client.GetAsync($"/api/v1/calendar?rentalId={postRentalResult.Id}&start={DateTime.UtcNow:yyyy-MM-dd}&nights=5"))
            {
                ...
                Assert.Single(getCalendarResult.Dates[3].PreparationTimeInDays);
                Assert.Equal(1, getCalendarResult.Dates[3].PreparationTimeInDays[0].Unit);

                Assert.Single(getCalendarResult.Dates[4].PreparationTimeInDays);
                Assert.Equal(2, getCalendarResult.Dates[4].PreparationTimeInDays[0].Unit);
            }
        }
    }
}
